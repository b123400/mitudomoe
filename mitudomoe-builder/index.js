...
var Coffee, Context, Path, Q, build, getAllItemsInContext, injectFramework, map, options, processItem, result;
...
build = function(options) {
  ...
  return getAllItemsInContext(options["in"]).then(map(function(item) {
    ...
    configure(app);
    return app.preBuild(options.out);
  });
};
    ...
    case 'folder':
      return Q.all((function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          item = _ref[_i];
          _results.push(processItem(item, options));
        }
        return _results;
      })());
    case 'file':
      if (Path.extname(item.name) === '.coffee') {
